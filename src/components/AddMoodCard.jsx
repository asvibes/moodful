...
export function AddMoodCard({ onAdd }) {
  ...
  const cancel = () => { setOpen(false); setLabel(""); setPreview(null); setImageData(null); };
  if (open) return (
    <div style={{ borderRadius: 24, border: "2px dashed #333", aspectRatio: "3/4", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 16, background: "#1a1a20", gap: 10, overflow: "hidden", position: "relative" }}>
      {preview && <img src={preview} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", opacity: 0.25, borderRadius: 24 }} />}
      <div style={{ position: "relative", zIndex: 1, width: "100%", display: "flex", flexDirection: "column", gap: 10, alignItems: "center" }}>
        ...
        <input value={label} onChange={(e) => setLabel(e.target.value)} placeholder="Mood name..."
          style={{ width: "100%", padding: "8px 12px", borderRadius: 10, border: "2px solid #333", fontSize: 13, outline: "none", boxSizing: "border-box", background: "#0F0F13", color: "#fff", fontFamily: "Lora, Georgia, serif" }} />
        ...
        <div style={{ display: "flex", gap: 8, width: "100%" }}>
          <button onClick={cancel} style={{ flex: 1, padding: "7px 0", borderRadius: 10, border: "2px solid #333", background: "transparent", fontSize: 12, cursor: "pointer", color: "#aaa" }}>Cancel</button>
