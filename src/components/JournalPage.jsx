...
export function JournalPage({ mood, onBack }) {
  ...
  const addBox = () => {
    const b = { id: Date.now(), text:"", x:80+Math.random()*200, y:120+Math.random()*100, w:400, bgColor:"rgba(10,10,16,0.75)", textColor:"#e0e0e0", accentColor: mood.color };
    saveBoxes([...boxes, b]);
  };
  ...
  const save = (boxId) => {
    ...
    setEntries(addJournalEntry(mood.id, box.text.trim(), box.x, box.y+200));
    updateBox(boxId, { text:"" });
  };
        ...
        <div style={{position:"sticky",top:0,zIndex:50,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 20px",background:"rgba(255,255,255,0.07)",backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",borderBottom:"1px solid rgba(255,255,255,0.1)",boxShadow:"0 4px 30px rgba(0,0,0,0.2)"}}>
          ...
          <div style={{display:"flex",gap:6,flexShrink:0}}>
            <button onClick={addBox}
              style={{background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:10,padding:"7px 14px",color:"#fff",fontSize:13,cursor:"pointer",fontFamily:"Fraunces,Georgia,serif",fontWeight:600}}>
              + Box
            </button>
            <button onClick={()=>setSettingsOpen(!settingsOpen)}
              style={{background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:10,padding:"7px 12px",color:"#fff",fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:5,fontFamily:"Fraunces,Georgia,serif",fontWeight:600}}>
    ...
    <DraggableItem initialX={box.x} initialY={box.y} onMove={(x,y)=>onMove(box.id,x,y)}>
      <div style={{width:box.w+"px",position:"relative",borderRadius:20,overflow:"visible"}}>
        ...
        <div style={{borderRadius:20,background:"rgba(255,255,255,0.06)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",border:"1px solid rgba(255,255,255,0.13)",boxShadow:"0 8px 40px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,255,255,0.1)",overflow:"hidden",position:"relative"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px 8px",background:"rgba(255,255,255,0.04)",borderBottom:"1px solid rgba(255,255,255,0.08)",cursor:"grab"}}>
            ...
            <div style={{display:"flex",gap:5}}>
              ...
              <button onClick={(e)=>{e.stopPropagation();onRemove(box.id);}}
                style={{background:"rgba(220,38,38,0.25)",border:"1px solid rgba(220,38,38,0.3)",borderRadius:7,padding:"3px 9px",fontSize:11,color:"#ff8888",cursor:"pointer"}}>
                x
              </button>
            </div>
          </div>
          ...
          {/* Write area */}
          <div style={{padding:"12px 14px",background:box.bgColor||"rgba(10,10,16,0.75)"}}>
            <textarea value={box.text||""} onChange={(e)=>onUpdate(box.id,{text:e.target.value})}
              placeholder="Write freely..."
              rows={4}
              style={{width:"100%",boxSizing:"border-box",padding:"10px 12px",borderRadius:12,border:"2px solid rgba(255,255,255,0.06)",fontFamily:"Lora,Georgia,serif",fontSize:15,lineHeight:1.75,color:box.textColor||"#e0e0e0",background:"rgba(0,0,0,0.2)",resize:"none",outline:"none",transition:"border-color 0.2s"}}
